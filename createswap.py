...
usage = "%prog -v vm_name -o vbox_user_home (full path) -n swap_name [ -s size ] [ -c configuration_file ]"
description = "Create a dinamic vdi file for swap"
version="%prog 0.1"
...
if options.conf:
    print ("Adding swapuuid=" + output[output.find("UUID: ") + 6:].strip() + " to " + options.conf ) 
    cp = ConfigParser()
    conf = options.conf
    cp.read(conf)
    cp.set("vm", "swapuuid", output[output.find("UUID: ") + 6:].strip())
